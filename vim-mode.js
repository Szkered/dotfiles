...
document.addEventListener('keydown', function (e) {
  if ((e.key === 'j' || e.key === 'k' || e.key === 'u' || e.key === 'd') && !isLinkKeyMode && !isCurrentlyInInput()) {
    if (e.key === 'j') {
...
const commandChars = new Set(['f', 'F', 'y', 'g', 'G', 'c', 'u', 'd'])
  ...
  if (e.key === 'Escape' && isLinkKeyMode) {
    ...
    switch (command) {
      ...
      case 'c':
         ...
         break
      // Use u to scroll down faster
      case 'u':
          window.scrollBy(0, -180)
          break
      // Use d to scroll up faster
      case 'd':
          window.scrollBy(0, 180)
          break
